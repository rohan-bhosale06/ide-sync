...
import ora from 'ora';
import { readConfig, readLastSyncedState, writeLastSyncedState, readConfigSyncConfig } from 'ide-sync-core';
import { runDetectors, ALL_FAMILIES } from 'ide-sync-core';
import type { IDEFamily } from 'ide-sync-core';
import { buildInstalledSet, hashState } from 'ide-sync-core';
import { threeWayMerge, applyRemotePlan, stampDevice } from 'ide-sync-core';
import { hasUnresolvedConflicts } from 'ide-sync-core';
import { formatConflictReport } from '../utils/conflict-report.js';
import { createBackend, PushConflictError } from 'ide-sync-core';
import { acquireLock } from 'ide-sync-core';
import type { MergePlan } from 'ide-sync-core';
import { configPush } from 'ide-sync-core';
import { resolveProfileIds } from 'ide-sync-core';
...
export async function runPush(opts: PushOptions = {}): Promise<PushResult> {
  ...
  const conflictPolicy = (opts.conflict ?? config.conflictPolicy) as import('ide-sync-core').ConflictPolicy;
